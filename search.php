...
<title>Search Results <?php echo $searchTerm ?> &raquo; Books</title>
...
<div id="container">
		...
		<div class="search">
			<form method="get" action="/books/search.php">
				<label class="hidden" for="s">
					<?php
						if (!isset($searchTerm)) {
							echo '<input type="text" value="" size="40" name="s" id="s" />';
						} else {
							echo '<input type="text" value="'.$searchTerm.'" size="40" name="s" id="s" />';
						}
					?>
	...
	<div id="content">
		<?php
			...
			$books = $db->fetchAll("select * from book where title like '%$searchTerm%'");
			
			// 分页
			$totalCount = count($books);
			$itemPerPage = 4;
		?>
  		<h3 class="title">“<span class="tag"><?php echo $searchTerm ?></span>”的搜索结果 [<?php echo $totalCount ?>]</h3>
		...
		<div class="navigation">
			<?php
				...
				// has previous page?
				if ($pageNo > 1) {
					echo '<a href="/books/search.php?s='.$searchTerm.'&p='.$prevPageNo.'">&laquo; 上一页</a>';
				}
				// is there more?
				if (($totalCount - 1) > $pageNo * $itemPerPage) {
					echo '<a href="/books/search.php?s='.$searchTerm.'&p='.$nextPageNo.'">&raquo; 下一页</a>';
				}
			?>
		</div>
	</div>
